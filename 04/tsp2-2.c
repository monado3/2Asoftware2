...
double solve(const int n, int ansroute[], double** distances) // simulated annealing
{
   ...
   ans = routedis(n, improvedsol, distances);
   while(temperature > EPSILON) {
      ...
      if(((double) rand() / RAND_MAX) <= exp((ans - tempvalue) / temperature)) {
         ans = tempvalue;
