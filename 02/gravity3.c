...
typedef struct star {
   double m;            // mass
   double x;            // position_x
   double xc;           // position_x after dt/2 (middle point)
   double vx;           // velocity_x
   double y;            // position_y
   double yc;           // position_y after dt/2 (middle point)
...
const double bm = 100.0; // mass of blackhole
...
void update_middlepoint(const double dt)    // 　dt/2後のx,yを求める。
{
   int i;
   for(i = 0; i < nstars; i++) {
...
void generate_star(void) // 星の生成
{
   ...
   printf("x=%f y=%f vx=%f vy=%f\n",stars[nstars].x,stars[nstars].y,stars[nstars].vx,stars[nstars].vy);
   nstars++;
}
