...
double f_value(const double x[])
{
   ...
   for(int sample = 0; sample < nsamples; sample++) {
      value += pow(data[sample].temp - (x[0] * data[sample].alt + x[1]), 2);
   }
   return value;
}
...
void f_gradient(const double x[], double g[])
{
   ...
   for(int sample = 0; sample < nsamples; sample++) {
      g[0] += -2 * (data[sample].temp - x[0] * data[sample].alt - x[1]) * data[sample].alt;
      g[1] += -2 * (data[sample].temp - x[0] * data[sample].alt - x[1]);
   }
}
